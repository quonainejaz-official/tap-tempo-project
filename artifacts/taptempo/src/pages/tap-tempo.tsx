...
import type { TapData } from "@/hooks/use-tap-tempo";
...
function TapGraph({ taps }: { taps: TapData[] }) {
  ...
  // Y range: pad ±20% around min/max so spikes are visible
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl border bg-card/50">
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={handleReset} disabled={!bpm} data-testid="btn-reset">Reset</Button>
              <Button variant="outline" size="sm" onClick={copyBpm} disabled={!bpm} data-testid="btn-copy">
                <Copy className="w-4 h-4 mr-2" /> Copy
              </Button>
              <Drawer>
                <DrawerTrigger asChild>
                  <Button variant="outline" size="sm" disabled={!bpm} data-testid="btn-history">
