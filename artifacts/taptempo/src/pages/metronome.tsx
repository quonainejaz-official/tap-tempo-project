...
function calcBpm(timestamps: number[]): number | null {
  ...
  // Outlier rejection: discard intervals > 2.5x the median
  const sorted = [...intervals].sort((a, b) => a - b);
  const median = sorted[Math.floor(sorted.length / 2)];
  const filtered = intervals.filter(v => v < median * 2.5);
  if (filtered.length === 0) return null;
  // Weighted average: recent intervals count 2x
  let weightedSum = 0;
  let weightTotal = 0;
...
export default function MetronomePage() {
  ...
  const fireTap = useCallback(() => {
    ...
    // Check if last tap was too long ago → reset
    ...
    if (calculated !== null) {
      ...
      // Immediately apply to metronome
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {["2/4", "3/4", "4/4", "5/4", "6/8", "7/8"].map(sig => (
              <Button
                ...
                size="sm"
                data-testid={`button-sig-${sig.replace("/", "-")}`}
